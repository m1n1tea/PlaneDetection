...
namespace VPCluster {
	...
	float calc_weight(std::list<sPtLnk *> mDataPoints, const std::vector<float>& vp){
		float sum = 0;

		for(sPtLnk *dp : mDataPoints){
			float dist = DistanceFunction_VP(vp,*(dp->mCoord));
			sum+=dist*dist;
		}
		return sum;
	}
		...
		) 
	{
		...
		modelIndex.resize(LabelCount.size());
		...
		for(std::list<sClLnk *>::iterator iterCl = mClustersList.begin(); iterCl != mClustersList.end(); ++iterCl){
			...
			for(int i=0; i<(*iterCl)->mPreferenceSet.size();++i){
				if((*iterCl)->mPreferenceSet[i]){
					float weight = calc_weight((*iterCl)->mBelongingPts,*(*mModels)[i]);
					if(weight<min_weight){
						min_weight=weight;
						modelIndex[counterCl]=i;
					}
